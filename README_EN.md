...
|---|---|---|---:|---|
| Minecraft 1.21.1 · NeoForge | Client required, server optional | Sodium 0.8.x / Iris 1.8.12+ | 21 | `neo-voxy-0.5.0-alpha.12-mc1.21.1-neoforge.jar` |
...
## Distant model detail (1.21.1)

Render Quality controls distant detail for Domum, opaque Copycat/LittleTiles, Create trains, bogeys, tracks, contraptions, kinetic parts and PowerGrid wires. Zooming in or increasing resolution restores detail; nearby models and the vanilla handoff area retain full geometry.

Dedicated coarse meshes always use native Voxy section boundaries and subdivision rules, with no separate toggle. Models spanning sections retain the finest touched level. Actual mesh and terrain levels can temporarily differ while terrain data is loading.

Dedicated translucent models retain transparent layers; beacons and lasers keep their thin shapes. Sable retains native full geometry. F3 shows submitted triangle counts and coarse/full draws through `Distant mesh triangles` and `Distant mesh draws coarse/full`.
